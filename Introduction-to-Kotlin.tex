...
Package-level functions are top-level functions (\autoref{sec:program-entry-point}) which are defined inside a package. These functions including extension functions will be converted to static methods inside a new Java class~\cite{interop-package-level-func} named after the Kotlin file the function oriented from, because methods outside a class are prohibited in Java. For example, consider a Kotlin file named \texttt{SalesPerson.kt} in the package \texttt{org.company}, containing the following package-level method:
\begin{lstlisting}[language=Kotlin]
// SalesPerson.kt
package org.company
...
Kotlin can be compiled not only for the JVM but also to native binaries, eliminating the need for a virtual machine as required by Java. This makes Kotlin suitable for use in embedded systems, where running a large virtual machine is not possible. Besides Java, Kotlin is also interoperable C and Swift/Objective-C and can be compiled to pure JavaScript or WebAssembly. Because Kotlin can be compiled to multiple targets, it is well suited for multiplatform development. To further simplify the process of writing code for multiple applications across various platforms, Kotlin offers a hierarchical project structure. In this structure, a common codebase can be defined and shared across subprojects for specific platforms. Each subproject can then use this shared codebase alongside its required platform-specific libraries, thus enhancing code maintainability and reducing duplication.

With Kotlin's Multiplatform programming support, you can develop applications for servers, desktop, web, and mobile, making it a key feature of the Kotlin programming language. 
% Ab hier würde ich den rest einfach weg cutten, und den platz für android nutzen
...
% means splitting your Hardware among multipile programms like multiple computers working at once
\section{Conclusion}
