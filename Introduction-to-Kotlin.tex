...
\subsection{Null Safety}
  ...
  In Kotlin, every type is non-nullable by default, so variables are considered non-nullable unless explicitly declared otherwise. To declare a variable as nullable, a question mark has to be added to the type.\footnote{This works with both mutable and immutable variables.}

  \begin{lstlisting}[language=Kotlin]
    var a: String = "a is non-nullable"
    var b: String? = "b is nullable"
  \end{lstlisting}
  Trying to assign null to a non-nullable variable will result in a compile-time error.
