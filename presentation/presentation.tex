...
\begin{frame}[fragile]{Main-Methode}
  \begin{lstlisting}[language=Java, title=Java Main-Methode, xleftmargin=2em]
public class Main {
  ...
  \end{lstlisting}
  \pause\vspace{0cm}
  \begin{lstlisting}[language=Kotlin, title=Kotlin Main-Methode, xleftmargin=2em]
fun main() {
    println("Hello, World!")
}
  \end{lstlisting}
    \pause\vspace{0cm}
    \begin{itemize}
      \item Keine explizite Klassendeklaration erforderlich. %Notes: Methoden auserhalb einer Klasse sind quasi statisch, aber ohne klassenzugehörigkeit. (Siehe auch println)
...
\begin{frame}[fragile]{Variablen Deklaration}
  \begin{columns}
    \begin{column}{0.5\textwidth}
      \begin{lstlisting}[language=Java, title=Java, xleftmargin=1.2em]
int a = 5;
final String b = "Hallo";
      \end{lstlisting}
    \end{column}
    \begin{column}{0.5\textwidth}
      \begin{lstlisting}[language=Kotlin, title=Kotlin, xleftmargin=1.2em, numbers=none]
var a: Int = 5
val b: String = "Hallo"
      \end{lstlisting}
    \end{column}
  ...
  \begin{itemize}
    ...
    \item Beispiel: \lstinline[language=kotlin]|var a = 5| ist auch möglich. % Dieses beispiel weglasen und oben im code block per animation einfügen?
